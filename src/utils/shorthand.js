...
export function buildShorthand(name, { tf, mhDay, down, subMap, curtisOffice, swap519, cycleDay }) {
  ...
  const scMH  = `Scrap→${mh}`                       // HSV (507) + Decatur (506) always scrap to MH
  const sc594 = `Scrap→${p("594", down, subMap)}`    // Muscle Shoals (519) scrap to Cherokee on non-BP
  ...
  // All crews take scrap to MH on BP days (519 only goes to MH on BP)
  if (onBP) {
    const postBP =
  ...
  if (C506_NAMES.includes(name)) {
    ...
    if (assigned === "514") return `${name}: ${scMH} 67s→${p("511",down,subMap)} rock→POD sand→${sub} scrap→LQ→RG 67s→${p("507",down,subMap)}→MM 67s→${p("511",down,subMap)}→POD sand→home`
